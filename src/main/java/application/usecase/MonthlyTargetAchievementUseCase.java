...
public class MonthlyTargetAchievementUseCase implements TargetAchievementUseCase {
	...
	@Override
	public LocalDate calTargetAchievement(int targetAmount, TargetDurationCalculator monthlyInvestmentAmount) {
		if (targetAmount <= 0) {
			throw new IllegalArgumentException("목표 달성 금액은 0보다 커야 합니다.");
		}
		int months = monthlyInvestmentAmount.calMonthsToReachTarget(targetAmount);
